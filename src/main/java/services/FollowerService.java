        ...
        if (replicas.size() > 0 && replicas.size() < branching) {
            Callable<Void> task = () -> {
                for (FollowerService replica : replicas) {
                    replica.writeAsync(key, value, version, new ArrayList<>(), thisService);
                }
                return null;
            };
            ...
            List<FollowerService> nextWriters = new ArrayList<>();
            List<List<FollowerService>> branches = new ArrayList<>();
